...
function onScroll() {
    ...
    $('.nav-fixed a[href^="#"]').each(function() {
        ...
        if ((scroll + header) >= offset && ($(el).outerHeight() + offset) > (scroll + header)) {
            ...
            $('.nav-fixed a[href^="index.html"]').removeClass('active');
            ...
            // if(!$('.nav-fixed a').hasClass('active')){
            //   $('.nav-fixed a[href^="index.html"]').addClass('active');
            // }
        }
    })
    //sp hamburger
